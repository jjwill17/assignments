...
getList.addEventListener("click", function(event){
    ...
    axios.get(`https://api.vschool.io/${inputText.value}/todo/`).then(function(response){
        ...
        for(var i = 0; i < todoList.length; i++){
            ...
            text += `ID Number: ${todoList[i]._id}<br>Title: ${todoList[i].title}<br>Description: ${todoList[i].description}<br>Price: ${todoList[i].price}<br><br>`
...
button.addEventListener("click", function(event){
    ...
    axios.post(`https://api.vschool.io/${addTodo.value}/todo/`, newTodo).then(function(response){
        ...
        var text = `New ToDo added:<br>Title: ${inputTitle.value}<br>Description: ${inputDescr.value}<br>Price: ${inputPrice.value}<br><br>`
